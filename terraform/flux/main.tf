...
resource "kubernetes_role_binding" "psp" {
  ...
  subject {
    kind      = "Group"
    name      = "system:serviceaccounts:${kubernetes_namespace.flux.metadata.0.name}"
    namespace = "${kubernetes_namespace.flux.metadata.0.name}"
  }
}
...
resource "kubernetes_deployment" "flux" {
  ...
  depends_on = ["kubernetes_cluster_role_binding.flux",
    "kubernetes_secret.flux-git-deploy",
    "kubernetes_cluster_role_binding.flux",
    "kubernetes_role_binding.psp",
  ]
  ...
  spec {
    ...
    template {
      ...
      spec {
        ...
        container {
          image = "${var.kubernetes_flux_image}:${var.kubernetes_flux_version}"
          name  = "flux"
...
resource "kubernetes_service" "flux" {
  ...
  metadata {
    ...
    labels = "${merge(map("name","${local.basename}", "app", "flux"), local.labels)}"
  }

  spec {
    selector {
      name = "${kubernetes_deployment.flux.metadata.0.labels.name}"
    }
